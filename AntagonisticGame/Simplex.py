    ...
    def function_is_limited(self, column):    # проверка ограниченности функции
        ...
        for i in range(self.M - 1):
            if self.Table[i, column] == 0:
                continue
            if self.Table[i, self.N - 1] / self.Table[i, column] >= 0:
                lim = True
                break
    ...
    def support_column_b(self, row):    # разрешающий столбец для поиска допустимого решения
        ...
        for j in range(self.N - 1):
            if self.Table[row, j] < 0:
                ...
                break
        for j in range(self.N - 1):
            if self.Table[row, j] < 0 and self.Table[row, j] < self.Table[row, column]:
                column = j
    ...
    def print_info(self):
        ...
        for i in range(self.M - 1):
            ...
            for j in range(self.N):
                print("{: >8.3f}".format(round(self.Table[i, j], 3)), end="")
            print()
